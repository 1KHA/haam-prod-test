  ...
  return (
    <div className="space-y-6 text-right">
      <div className="flex items-center justify-between">
        <div>{unreadCount > 0 && <Button variant="outline" size="sm" onClick={markAllRead}><Check className="h-4 w-4 ml-2" />تحديد الكل كمقروء</Button>}</div>
        <div><h1 className="text-3xl font-bold">الإشعارات</h1>{unreadCount > 0 && <p className="text-sm text-muted-foreground">{unreadCount} إشعار غير مقروء</p>}</div>
      </div>
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2 justify-end"><span>جميع الإشعارات</span><Bell className="h-5 w-5" /></CardTitle></CardHeader>
        <CardContent>
          {loading ? (
            <div className="flex justify-center py-8"><RefreshCw className="h-6 w-6 animate-spin" /></div>
          ) : notifications.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground"><Bell className="h-12 w-12 mx-auto mb-3 opacity-30" /><p>لا توجد إشعارات</p></div>
          ) : (
