    ...
    <div className="space-y-6 text-right">
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          <Link href="/admin-dashboard/finance/export">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{financeSummary.totalFunding.toLocaleString()} ريال</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{financeSummary.totalPayments.toLocaleString()} ريال</div>
            <div className="flex items-center mt-2 text-green-600">
              <ArrowUpRight className="h-4 w-4 mr-1" />
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{financeSummary.netRevenue.toLocaleString()} ريال</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{financeSummary.completedDeals}</div>
            <div className="text-sm text-muted-foreground mt-1">{financeSummary.pendingDeals} صفقات معلقة</div>
          </CardContent>
        </Card>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Link href="/admin-dashboard/finance/funding" className="block">
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-md border p-3">
                  ...
                  <div className="text-2xl font-bold mt-1">{financeSummary.completedDeals}</div>
                </div>
                <div className="rounded-md border p-3">
                  <div className="text-sm font-medium text-muted-foreground">متوسط التمويل</div>
                  <div className="text-2xl font-bold mt-1">{(financeSummary.avgInvestment / 1000000).toFixed(1)}M ريال</div>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Link href="/admin-dashboard/finance/payments" className="block">
          <Card className="h-full transition-all hover:shadow-md">
            <CardHeader>
              <CardTitle className="flex items-center justify-end gap-2">
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-md border p-3">
                  ...
                  <div className="text-2xl font-bold mt-1">{financeSummary.completedPayments}</div>
                </div>
                <div className="rounded-md border p-3">
                  <div className="text-sm font-medium text-muted-foreground">المدفوعات المعلقة</div>
                  <div className="text-2xl font-bold mt-1">{financeSummary.pendingPayments}</div>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex justify-between items-center">
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="flex items-center gap-1" onClick={() => window.location.href = '/admin-dashboard/finance/export'}>
                <Download className="h-4 w-4" />
                <span>تصدير</span>
              </Button>
            </div>
                      ...
                      <div className="flex gap-1">
                        <Link 
                          href={transaction.type === "funding" 
                            ? `/admin-dashboard/finance/funding/${transaction.id}` 
                            : `/admin-dashboard/finance/payments/${transaction.id}`}
                        >
                        ...
                        <Link 
                          href={transaction.type === "funding" 
                            ? `/admin-dashboard/finance/funding/${transaction.id}/edit` 
                            : `/admin-dashboard/finance/payments/${transaction.id}/edit`}
                        >
